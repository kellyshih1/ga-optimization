...
class Config:
    def __init__(self, dimension, representation, population_size, uniform_crossover, cross_prob, mut_prob, num_generations, tournament_size, debug):
...
def parse_arguments():
    ...
    parser.add_argument('-k', '--tournament_size', type=int, default=2, help='Number of samples in tournament selection (default: 2)')
    parser.add_argument('-d', '--debug', action='store_true', help='Turn on debug prints (default: false)')

    args = parser.parse_args()
    
    return Config(dimension=args.dimension, representation=args.representation, population_size=args.population_size, uniform_crossover=args.uniform_crossover, cross_prob=args.pc, mut_prob=args.pm, num_generations=args.generations, tournament_size=args.tournament_size, debug=args.debug)